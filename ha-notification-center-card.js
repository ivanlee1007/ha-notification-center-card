  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .panel-controls .icon-control-btn {
          width: 34px;
          height: 34px;
          min-width: 34px;
          padding: 0;
        ...
        .panel-controls .icon-control-btn ha-icon { --mdc-icon-size: 18px; }
  ...
  _renderPanelControls(count, t) {
    return `
      <div class="panel-controls">
        <button class="auto-clear-settings-btn icon-control-btn" type="button" title="${t("autoClearSettings")}" aria-label="${t("autoClearSettings")}"><ha-icon icon="mdi:timer-cog-outline"></ha-icon></button>
        ${count > 0 ? `<button class="clear-all-btn icon-control-btn" type="button" title="${t("clearAll")}" aria-label="${t("clearAll")}"><ha-icon icon="mdi:bell-remove-outline"></ha-icon></button>` : ""}
      </div>
    `;
  }
...
class NotificationChipCard extends HTMLElement {
  ...
  _handleAction(sourceId, e) {
    e.stopPropagation();
    const item = this.shadowRoot.querySelector(`.notif-content[data-source="${sourceId}"]`);
