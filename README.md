...
| `default_open` | `false` | 卡片載入後預設展開 |
| `auto_open_critical` | `false` | 偵測到緊急事件時自動展開 |
...
│   └── ha-notification-center-card.js.gz
├── ha-notification-center-card.js      # HACS 實際服務的主檔（與 dist 同步）
├── ha-notification-center-card.js.gz
├── notification-chip-card.js
├── notification-chip-card.js.gz
├── LICENSE
├── README.md
└── hacs.json
```
